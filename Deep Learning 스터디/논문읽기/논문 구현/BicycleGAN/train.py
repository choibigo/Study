...
def TrainRun(**kwargs):
    ...
    train_dataset = BicycleGANDataset(train_path_list,
    ...
    if len(valid_path_list[0]) != 0:
        valid_flag = True
        valid_dataset = BicycleGANDataset(valid_path_list,
                                          hyperparameter['load_size'],
                                          hyperparameter['crop_size'],
                                          hyperparameter['input_channel'],
                                          hyperparameter['output_channel'],
    ...
    model = BiCycleGANModel(hyperparameter['lr'],
                            hyperparameter['batch_size'],
                            hyperparameter['input_channel'],
                            hyperparameter['output_channel'],
                            hyperparameter['latent_vector_size'],
                            device)
        ...
        if valid_flag:
            with torch.no_grad():
                for valid_n_count, valid_data in enumerate(valid_loader):
                    z = torch.randn(valid_data['A'].size(0), hyperparameter['latent_vector_size'])
                    fake_b = model.netG(valid_data['A'].to(device), z.to(device))
                    
                    valid_pred_fake = model.netD(fake_b) 
                    loss_valid_G_GAN, _ = model.criterionGAN(valid_pred_fake, True)
                    ...
                    cv2.imwrite(f"{save_base_path}\\bicycle_gan_temp\\{epoch}_{valid_n_count}_predict.png", cv2.cvtColor(save_image, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(f"{save_base_path}\\bicycle_gan_temp\\{epoch}_{valid_n_count}_input.png", cv2.cvtColor(input_image, cv2.COLOR_RGB2BGR))
...
def download_image(train_ratio):
    ...
    for path in os.listdir(os.path.join(root, "gds")):
        A_paths.append(os.path.join(root, 'sem', path))
        B_paths.append(os.path.join(root, 'gds', path))
